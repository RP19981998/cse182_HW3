...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        #[batch_size, heads, n_queries, depth_v]
        ####################################  END OF YOUR CODE  ##################################
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, input_shapes):
        ...
        #它们分别表示查询（query）和键值（key-value）的通道数。
        self.n_heads = n_heads
        ...
        #nn.linear全连接层,输入和输出维度相同，不使用bias
        #权重归一化可以帮助网络更快的训练并提高模型的泛化能力，特别对很深的神经网络
        ...
        #----------------------#
        #这是一个自定义的函数，用于对线性层的权重使用Xavier方法初始化
        #可帮助训练神经网络模型时更好地收敛。
        def weights_init(m):

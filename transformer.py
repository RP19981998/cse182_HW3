...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        seq_pos = th.arange(start, sequence_length + start, dtype=th.float32)#inputs.device
        seq_pos_expanded = seq_pos[None,:,None]

        #序列将在第一个维度上重复一次，在第二个维度上重复一次，并且在第三个维度上重复self.hidden_size//2次。
        index = seq_pos_expanded.repeat(*[1,1,self.hidden_size//2])#[1,sequence_length,hidden/2]
        ...
        position_embedding = th.stack((sin_embedding,cos_embedding), dim=-1).view(position_shape)
    ...
    def __init__(self, input_size,
        ...
        def weights_init(m):
            if isinstance(m, nn.Linear):
                ...
                #使用了Xavier初始化方法，确保权重的初始值适合神经网络的训练。
...
class Transformer(nn.Module):
    ...
    def forward(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True, shift_target_sequence_right=True):
        ...
        shift_target_sequence_right,mask_future)
